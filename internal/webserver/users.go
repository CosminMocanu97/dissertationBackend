...
const (
	...
	ERROR_PHONE_NUMBER_IS_INVALID = "Numarul de telefon este invalid"
)
...
func (s *Service) HandlePostRegisterRequest(c *gin.Context) {
	...
	phoneNumberIsValid := utils.ValidatePhoneNumber(registrationData.PhoneNumber)
	if !phoneNumberIsValid {
		c.JSON(http.StatusBadRequest, gin.H{
			"error": ERROR_PHONE_NUMBER_IS_INVALID,
		})
		return
	}

	log.Info("%s; %t", registrationData.PhoneNumber, phoneNumberIsValid)

	activationToken := utils.GenerateRawAccountActivationToken()
	log.Info("UUID: %s; The registrationData is user: %s, password: %s, phone: %s",
		snowflakeID, registrationData.Email, registrationData.Password, registrationData.PhoneNumber)
	...
	if gsErr != gserror.NoError {
		log.Error("UUID: %s; Error adding the user %s with the password %s and phone number %s: %s",
			snowflakeID, registrationData.Email, registrationData.Password, registrationData.PhoneNumber, gsErr)
		...
		if !userAdded {
			...
			err = s.MailingService.SendEmail(recipients, "Activare cont", activationToken, "", snowflakeID)
			if err != nil {
...
func (s *Service) HandlePostForgotPasswordRequest(c *gin.Context) {
	...
	if gsErr != gserror.NoError {
		...
		recipients := []string{registrationData.Email}
		err = s.MailingService.SendEmail(recipients, "Resetati parola", newTokenWithUserId, "", snowflakeID)
		if err != nil {
