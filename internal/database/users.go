...
func AddNewUser(db *sql.DB, registrationData types.RegistrationData, activationToken string, id snowflake.ID) (bool, gserror.GSError) {
	...
	if err != gserror.NoError {
		...
		if !userExists {
			...
			addUserStatement :=
				"INSERT INTO users(email, passHash, phoneNumber, activationToken) VALUES($1, $2, $3, $4);"

			_, err := db.Exec(addUserStatement, registrationData.Email, passHash, registrationData.PhoneNumber, activationToken)
			if err != nil {
...
func GetUserDetailsForEmail(db *sql.DB, email string, id snowflake.ID) (types.User, gserror.GSError) {
	...
	getUserIdForEmailQuery :=
		"SELECT id, passhash, isActivated, phoneNumber, activationToken FROM users WHERE email=$1"

	row := db.QueryRow(getUserIdForEmailQuery, email)
	err := row.Scan(&user.ID, &user.Passhash, &user.IsActivated, &user.PhoneNumber, &user.ActivationToken)
	if err != nil {
		if err == sql.ErrNoRows {
...
func UserIsActivated(db *sql.DB, email string, id snowflake.ID) (bool, gserror.GSError) {
	userIsVerifiedQuery := "SELECT activated FROM users WHERE email=$1"
